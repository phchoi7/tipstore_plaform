...
import { checkName, returnTeamIcon } from './AiScoreCard.function';
// components
  ...
  return (
    <Card>
      ...
      <Grid item sx={{ display: 'flex', justifyContent: 'space-around', textAlign: 'center' }}>
        <Box sx={{ p: 3, pb: 1, textAlign: '-webkit-center' }} dir="ltr">
          <Avatar alt="Remy Sharp" src={returnTeamIcon(data.homeTeam)} sx={{ width: 80, height: 80 }} />
        ...
        <Box sx={{ p: 3, pb: 1, textAlign: '-webkit-center' }} dir="ltr">
          <Avatar alt="Remy Sharp" src={returnTeamIcon(data.visitTeam)} sx={{ width: 80, height: 80 }} />
          <Typography variant="h4" sx={{ color: 'text.secondary' }}>
